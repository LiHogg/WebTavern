def apply_booking_flow_patch():
    ...
    def confirm_hold(self, request, pk=None):
        with transaction.atomic():
            ...
            if booking.customer_id != request.user.id:
                return response.Response({"detail": "Подтвердить резерв может только клиент, который его создал."}, status=status.HTTP_403_FORBIDDEN)
    ...
    def confirm(self, request, pk=None):
        with transaction.atomic():
            ...
            booking.hold_expires_at = None
            booking.save(update_fields=["status", "hold_expires_at", "updated_at"])
            BookingStatusHistory.objects.create(booking=booking, old_status=old_status, new_status=booking.status, changed_by=request.user, reason=reason)
        ...
        broadcast_notification({"type": "booking_confirmed", "booking_id": booking.id, "venue_id": booking.venue_id, "message": message}, venue_id=booking.venue_id)
        broadcast_booking_update(booking, event_type="booking_confirmed", message=message)
        return response.Response(_serialize(booking, request), status=status.HTTP_200_OK)

    BookingViewSet.confirm_hold = confirm_hold
    BookingViewSet.confirm = confirm
